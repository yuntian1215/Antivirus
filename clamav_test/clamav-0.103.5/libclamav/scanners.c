...
static cl_error_t cli_scanmail(cli_ctx *ctx)
{
	...
	//POST: inject
	//cli_errmsg("cli_scanmail tmp dir: %s\n", dir);
    /*
...
cl_error_t cli_magic_scan(cli_ctx *ctx, cli_file_t type)
{
    ...
    if (ctx->engine->keeptmp) {
		...
		//POST: inject magic_scan
		//cli_errmsg("magic_scan old tmpdir: %s\n", old_temp_path);
		//cli_errmsg("magic_scan new tmpdir: %s\n", new_temp_path);
	...
	//POST:判断文件的类型; eml file?
	filetype = cli_ftname(type); //enum to string!
	//cli_errmsg("file type: %s\n", filetype);
     ...
     */
	char * now_type = cli_ftname(type);
	//cli_errmsg("injected: %s\n", now_type);
    ...
    switch (type) {
        ...
        case CL_TYPE_HTML:
			...
			{
				//cli_errmsg("scan html start!\n");
				ret = cli_scanhtml(ctx);
				//cli_errmsg("scan html done!\n");
			}
            break;
        ...
        case CL_TYPE_MAIL:
			if (SCAN_PARSE_MAIL && (DCONF_MAIL & MAIL_CONF_MBOX))
			{
				//cli_errmsg("mail scan start!\n");
				ret = cli_scanmail(ctx);
				//cli_errmsg("mail scan done!\n");
			}
            break;
...
static cl_error_t scan_common(cl_fmap_t *map, const char *filepath, const char **virname, unsigned long int *scanned, const struct cl_engine *engine, struct cl_scan_options *scanoptions, void *context)
{
    ...
    ctx.sub_tmpdir = new_temp_path;
	//POST: inject print new temp path
	//cli_errmsg("layer1 tmp file: %s\n", new_temp_path);
    if (mkdir(ctx.sub_tmpdir, 0700)) {
