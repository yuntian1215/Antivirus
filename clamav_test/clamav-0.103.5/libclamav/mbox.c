...
parseEmailBody(message *messageIn, text *textIn, mbox_ctx *mctx, unsigned int recursion_level)
{
	...
	//Injected should here be change to doCheckURLs??
	const int doCheckUrls = (engine->dboptions & CL_DB_PHISHING_URLS && (DCONF_PHISHING & PHISHING_CONF_ENGINE)) || engine->dboptions & CL_DB_TRACKING_URLS;//判断使用engine的conf
#if HAVE_JSON
    ...
    if (mainMessage && (messageGetBody(mainMessage) != NULL)) {
...
#endif
		//POST: inject  subtype: should be multipart/mixed!
		//cli_errmsg("subtype: %s\n", mimeSubtype);
        /* pre-process */
        ...
        switch (mimeType) {
            ...
            case MULTIPART:
				...
				//cli_errmsg("multipart boundary: %s\n", boundary);
				//POST:inject multipart
#if HAVE_JSON
                if (mctx->wrkobj != NULL)
                ...
                for (multiparts = 0; t_line && !infected; multiparts++) {
					...
					//POST: inject amessage mime type 拿个amessage的mimetype出来看看。
					//cli_errmsg("amessage mime type: %s\n", messageGetMimeSubtype(aMessage));
				...
				//POST: inject multiparts:
				//cli_errmsg("multiparts: %d\n", multiparts);
                free((char *)boundary);
				...
				//现在就全存在mesages队列里了。
				//cli_errmsg("mime subtype bufore recursion: %s\n", mimeSubtype);
                /*
...
checkURLs(message *mainMessage, mbox_ctx *mctx, mbox_status *rc, int is_html)
{
    ...
    hrefs.scanContents = mctx->ctx->engine->dboptions & CL_DB_PHISHING_URLS && (DCONF_PHISHING & PHISHING_CONF_ENGINE) || mctx->ctx->engine->dboptions & CL_DB_TRACKING_URLS;
	...
	if (b) {
		if (hrefs.scanContents) {
			if (mctx->ctx->engine->dboptions & CL_DB_PHISHING_URLS) {
			...
			//if (mctx->ctx->engine->dboptions & CL_DB_TRACKING_URLS) {
				cl_error_t statuss = trackingScan(mctx->ctx, &hrefs);
				if (statuss != CL_CLEAN) {
					mainMessage->isInfected = true;
					*rc = VIRUS;
					cli_dbgmsg("TRACK: Traking found\n");
				}
			/*	else
				{
					cli_errmsg("trackingscan finish,but not found\n");
				}
*/
		}
	}
...
do_multipart(message *mainMessage, message **messages, int i, mbox_status *rc, mbox_ctx *mctx, message *messageIn, text **tptr, unsigned int recursion_level)
{
	...
	const int doCheckUrls =(mctx->ctx->engine->dboptions & CL_DB_PHISHING_URLS && (DCONF_PHISHING & PHISHING_CONF_ENGINE)) || mctx->ctx->engine->dboptions & CL_DB_TRACKING_URLS;
#if HAVE_JSON
        ...
        case TEXT:
            ...
            if ((*dtype == '\0') || (strcasecmp(dtype, "inline") == 0)) {
                ...
                if ((tableFind(mctx->subtypeTable, cptr) == PLAIN) &&
					...
					//mimesubtype 是html就执行checkURLs
					const int is_html = (tableFind(mctx->subtypeTable, cptr) == HTML);
					if (doCheckUrls)
						checkURLs(aMessage, mctx, rc, is_html);//POST: inject
