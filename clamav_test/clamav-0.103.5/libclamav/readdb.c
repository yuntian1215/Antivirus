...
//Injected: cli_loadtdb definition
static int cli_loadtdb(FILE *fs, struct cl_engine *engine, unsigned int *signo, unsigned int options, struct cli_dbio *dbio) 
{
	int ret = 0;
	//need to define a domain list matcher?but the thing is, it is for different usage?
	if (!engine->track_list_matcher) {
		if (CL_SUCCESS != (ret = init_track_list(engine))) {
			return ret;
		}
	}
	if (CL_SUCCESS != (ret = load_regex_matcher(engine, engine->track_list_matcher, fs, signo, options, 0, dbio, engine->dconf->other & OTHER_CONF_PREFILTERING))) {
		return ret;
	}
	return CL_SUCCESS;
...
cl_error_t cli_load(const char *filename, struct cl_engine *engine, unsigned int *signo, unsigned int options, struct cli_dbio *dbio)
{
	...
	else if (cli_strbcasestr(dbname, ".tdb")) {
			//Injected: cli_loadtdb
			if (options & CL_DB_TRACKING_URLS) {
				ret = cli_loadtdb(fs, engine, signo, options, dbio);
			}
			else
				skipped = 1;
...
cl_error_t cl_load(const char *path, struct cl_engine *engine, unsigned int *signo, unsigned int dboptions)
{
	...
	if (((dboptions & CL_DB_PHISHING_URLS) && !engine->phishcheck && (engine->dconf->phishing & PHISHING_CONF_ENGINE)) || dboptions & CL_DB_TRACKING_URLS) 
	{
		//Injected: dboptions
		// if(dboptions & CL_DB_TRACKING_URLS){
		//     cli_errmsg("cli_load_db_dir dboptions!\n");
		//     //return CL_ENULLARG;
		// }
		//return CL_ENULLARG;
...
cl_error_t cl_engine_compile(struct cl_engine *engine)
{
	...
	if ((ret = cli_build_regex_list(engine->track_list_matcher))) {
		return ret;
	}
